// opm_drv.cpp
// Midi->OPM Parameter and tone management
// 2002.3.11 sam
// based MiOPMdrv
...
void OPMDRV::setDelta(unsigned int Delta) { SampleTime = Delta; }

void OPMDRV::AutoSeq(int sampleframes) {
  // For portamento.
  // AutoSeq is called back from the OPM waveform rendering process.
  // At the start of the next rendering,
  // make all commands issued by this function be executed.
  int i;
  int Porta;
  int Cnt;
  class CHDATA *pCh;
...
int OPMDRV::getProgNum(void) { return ((int)CuProgNum); }
...
char *OPMDRV::getProgName(int index) { return (VoTbl[index].Name); }
...
void OPMDRV::setData(int ch, int data) {
  if (TblMidiCh[ch].CuRPN == true) {
    ...
    case 0x0000: // OPM clock setting
      ...
      break;
    case 0x0001: // Volume bias setting
      _iocs_opmset(0x5, data);
      break;
    case 0x0002: // LPF ON/OFF setting
      if (data <= 0x3f) {
        // ON:default
        _iocs_opmset(0x6, 0);
      } else {
...
void OPMDRV::SendKc(int ch) {
  ...
  if (Note < 0x80) {
    Word = (Note << 16) +
           ((TblMidiCh[MidiCh].Bend << 3) * TblMidiCh[MidiCh].BendRang) +
           TblCh[ch].PortaBend; // 16.16bit fixed-point
    switch (OpmClock) {
    case 1: // 4MHz
      // If OPM Clock is 4MHz, correct frequency.
      // 12*log2(4MHz/3.58MHz)*65536<16.16bitFixed-point offset>.
      Word -= 125861;
      break;
    case 2: // for 3.579545MHz correct frequency
            // 12*log2(4MHz/3.58MHz)*65536<16.16bit fixed-point offset>.
      Word += 144;
      break;
    }

    Note = Word >> 16;
    //	kc=(Ktbl[(Note-3)%12])+((int)((Note-3)/12)*16);
    // Calculation of Kc[Oct::key

    kc = (Ktbl[(Note - 1) % 12]) +
         ((int)((Note - 1) / 12) * 16); // Calculation of Kc[Oct::key
    kf = (Word >> 8) & 0xfc;     // calculation of Kf[Bend fine adjustment].
    _iocs_opmset(0x28 + ch, kc); // Write the tone scale to the OPM register
    _iocs_opmset(0x30 + ch, kf); // write the bend to the OPM register
  }
#ifdef MyDEBUG
...
OPMDRV::OPMDRV(class Opm *pTmp) {
  ...
  for (i = 0; i < CHMAX; i++) {
    TblCh[i].VoNum = 0;
    TblCh[i].MidiCh = i;
    TblCh[i].Note = 0xff; // Note:0xff Free channel.
    TblCh[i].Vel = 0x00;  // silencing
    TblCh[i].PortaBend = 0;
...
void OPMDRV::NoteOn(int MidiCh, int Note, int Vel) {
  ...
  if (TblMidiCh[MidiCh].PortaSpeed) {
    if ((TblMidiCh[MidiCh].PortaOnOff == true) ||
      ...
      TblCh[ch].PortaSpeed =
          (PortaDelta * 0x10000) / TblMidiCh[MidiCh].PortaSpeed;
  ...
  if (TblMidiCh[MidiCh].LFOdelay == 0) {
    ...
    _iocs_opmset(0x38 + ch, 0); // PMS/AMS LFO not applied
  }
  ...
  // Vel&Vol Set
  _iocs_opmset(0x02, TblMidiCh[TblCh[ch].MidiCh].Vol); // Vol extension set
  _iocs_opmset(0x04, TblCh[ch].Vel);                   // Vel extension set

  //_iocs_opmset(0x01, 2);	// Reset LFO.
  if (PolyMono == false && BfNote != 0xff) {
    //  No attack (legato) when pronounced in Mono mode.
  } else {
    _iocs_opmset(0x01, 0); // Start LFO.
    _iocs_opmset(0x08, (ch & 0x07) + (VoTbl[TblMidiCh[MidiCh].VoNum].OpMsk &
                                      0x78)); // Key on.
  }
...
void OPMDRV::VolCng(int MidiCh, int Vol) {
  ...
  for (i = 0; i < CHMAX; i++) {
    if (TblCh[i].MidiCh == MidiCh && TblCh[i].Note != 0xff) {
      _iocs_opmset(0x00, i);   // channel expansion set
      _iocs_opmset(0x02, Vol); // Vol extension set
    }
  }
}
...
int OPMDRV::Save(char *tmpName) {
  ...
  for (ch = 0; ch < VOMAX; ch++) {
    pCh = &(VoTbl[ch]);
    fprintf(File, "\n@:%d %s\n", ch, pCh->Name);
    fprintf(File, "LFO:%3d %3d %3d %3d %3d\n", pCh->SPD, pCh->AMD, pCh->PMD,
            pCh->WF, pCh->NFQ);
    fprintf(File, "CH:%3d %3d %3d %3d %3d %3d %3d\n", pCh->F_PAN, pCh->FL,
            pCh->CON, pCh->AMS, pCh->PMS, pCh->OpMsk, pCh->F_NE);
    for (i = 0; i < 4; i++) {
      j = TblS[i];
      pOp = &(pCh->Op[j]);
      fprintf(File, "%s:%3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d\n", name[j],
              pOp->AR, pOp->D1R, pOp->D2R, pOp->RR, pOp->D1L, pOp->TL, pOp->KS,
              pOp->MUL, pOp->DT1, pOp->DT2, pOp->F_AME);
    }
  }
  fclose(File);
  return (ERR_NOERR);
...
int OPMDRV::Load(char *tmpName) {
  ...
  while (fgets(line, 256, File) != NULL) {
    ...
    while (*pline != 0 &&
      ...
      // Skip space
      while (strncmp(" ", pline, 1) == 0 && pline != (256 + line)) {
        pline++;
      }
      ...
      if (strncmp("@:", pline, 2) == 0) {
        pline += 2;
        if (sscanf(pline, "%d", &CuVoNum) != 1) {
          return (ERR_LESPRM + Nline);
        }
        if (CuVoNum < 0) {
          CuVoNum = 0;
        }
      ...
      if (strncmp("CH:", pline, 3) == 0) {
        pline += 3;
        if (sscanf(pline, "%d %d %d %d %d %d %d", &(pCh->F_PAN), &(pCh->FL),
                   &(pCh->CON), &(pCh->AMS), &(pCh->PMS), &(pCh->OpMsk),
                   &(pCh->F_NE)) != 7) {
          return (ERR_LESPRM + Nline);
        }
        errMsk |= 0x02;
      ...
      if (strncmp("LFO:", pline, 4) == 0) {
        pline += 4;
        if (sscanf(pline, "%d %d %d %d %d", &(pCh->SPD), &(pCh->AMD),
                   &(pCh->PMD), &(pCh->WF), &pCh->NFQ) != 5) {
          return (ERR_LESPRM + Nline);
        }
        errMsk |= 0x04;
        while (isdigit(*pline) || *pline == ' ') {
      ...
      i = -1;
      if (strncmp("M1:", pline, 3) == 0) {
        i = 0;
        pline += 3;
        errMsk |= 0x10;
      }
      if (strncmp("M2:", pline, 3) == 0) {
        i = 1;
        pline += 3;
        errMsk |= 0x20;
      }
      if (strncmp("C1:", pline, 3) == 0) {
        i = 2;
        pline += 3;
        errMsk |= 0x40;
      }
      if (strncmp("C2:", pline, 3) == 0) {
        i = 3;
        pline += 3;
        errMsk |= 0x80;
      }
      if (i != -1) {
        ...
        if (sscanf(pline, "%d %d %d %d %d %d %d %d %d %d %d", &(pOp->AR),
                   &(pOp->D1R), &(pOp->D2R), &(pOp->RR), &(pOp->D1L),
                   &(pOp->TL), &(pOp->KS), &(pOp->MUL), &(pOp->DT1),
                   &(pOp->DT2), &(pOp->F_AME)) != 11) {
          return (ERR_LESPRM + Nline);
        }
        while (isdigit(*pline) || *pline == ' ') {
          pline++;
...
int OPMDRV::AtachCh(void) {
  ...
  NewCh = ChkEnCh();
  if (NewCh == -1) {
    // If there are no free channels, the first channel is made last.
    NewCh = PlayCh.front();
    PlayCh.pop_front();
  } else {
    WaitCh.pop_front();
...
int OPMDRV::ChkSonCh(int MidiCh, int Note) {
  // Find the slot being pronounced from MidiCh and velocity.
  // Note=0xff is a wildcard.
...
int OPMDRV::NonCh(int MidiCh, int Note, int Vel) {
  int ch;
  // MidiCh and note, pronounced with velocity.
  if (ChkSonCh(MidiCh, Note) != -1) {
    return (-1);
  } // Has it been pronounced?

  ch = AtachCh(); // Find and allocate usable slots from the list.
  TblCh[ch].Note = Note;
  TblCh[ch].Vel = Vel;
  TblCh[ch].MidiCh = MidiCh;
  TblCh[ch].VoNum = TblMidiCh[MidiCh].VoNum;
...
int OPMDRV::NoffCh(int MidiCh, int Note) {
  int ch;
  // silencing by MidiCh and notes.
  if ((ch = ChkSonCh(MidiCh, Note)) != -1) { // Is it silenced?
    DelCh(ch);
  }
...
unsigned char OPMDRV::getPrm(int index) {
  ...
  case kC2DT2:
    return (pVo->Op[3].DT2);
  case kM1F_AME:
    return ((pVo->Op[0].F_AME == 128) ? 1 : 0);
  case kM2F_AME:
    return ((pVo->Op[1].F_AME == 128) ? 1 : 0);
  case kC1F_AME:
    return ((pVo->Op[2].F_AME == 128) ? 1 : 0);
  case kC2F_AME:
    return ((pVo->Op[3].F_AME == 128) ? 1 : 0);
    // OpMsk
  case kMskM1:
    return ((pVo->OpMsk & 0x08) ? 1 : 0);
  case kMskC1:
    return ((pVo->OpMsk & 0x10) ? 1 : 0);
  case kMskM2:
    return ((pVo->OpMsk & 0x20) ? 1 : 0);
  case kMskC2:
    return ((pVo->OpMsk & 0x40) ? 1 : 0);
    // Con
  case kCon0:
  case kCon1:
  case kCon2:
...
void OPMDRV::setPrm(int index, unsigned char data) {
  ...
  case kC2DT2:
    ...
    break;
  case kM1F_AME:
    pVo->Op[0].F_AME = (data == 1) ? 128 : 0;
    break;
  case kM2F_AME:
    pVo->Op[1].F_AME = (data == 1) ? 128 : 0;
    break;
  case kC1F_AME:
    pVo->Op[2].F_AME = (data == 1) ? 128 : 0;
    break;
  case kC2F_AME:
    pVo->Op[3].F_AME = (data == 1) ? 128 : 0;
    break;
    // OpMsk
  case kMskM1:
    if (data == 0) {
