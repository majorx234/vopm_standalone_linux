...
Lfo::Lfo(int samplerate, int opmrate)
    : samplerate(samplerate), opmrate(opmrate) {
  int i;
  ...
  // AM Wave Form 0,3
  for (i = 0; i <= 255; ++i) {
    // 2006.3.12 Fix by sam LFO: To prevent petit noise when LFO delay is
    // applied.
    //  Fix phase
    /*
        AmTbl0[i] = 255-i;
  ...
  // AM Wave Form 2
  for (i = 0; i <= 255; ++i) {
    // 2006.3.12  Fix sam To prevent petit noise when LFO delay is applied
    //  Fix phase
    /*
        AmTbl2[i] = 255-i;
        AmTbl2[i+256] = i;
...
void Lfo::Init() {
  // 2008.01.15 sam Modified OPM CLOCK variable support
...
void Lfo::LfoStart() { LfoStartingFlag = 1; }
...
void Lfo::Update() {
  ...
  LfoTime += LfoTimeAdd;
  // 2008.4.19 sam Fix To reduce the error in LfoTime, the residual is stored.
...
int Lfo::GetPmValue(int ch) { return PmValue[ch]; }
int Lfo::GetAmValue(int ch) { return AmValue[ch]; }
